...
public class FacesUtil {
	...
	public static void addMessage(String message,boolean status) {
		if(status){
			FacesContext.getCurrentInstance().addMessage("info",new FacesMessage(FacesMessage.SEVERITY_INFO, message, message));
		}else{
			FacesContext.getCurrentInstance().addMessage("warning",new FacesMessage(FacesMessage.SEVERITY_WARN, message, message));
		}
